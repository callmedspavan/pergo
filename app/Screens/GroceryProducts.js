...
function GroceryProducts({ navigation, route }) {
  ...
  const [subPackModal, setSubPackModal] = useState(false);
  ...
  const loadProducts = async () => {
    setLoading(true);
    const productsresponse = await getGroceryProducts.getGroceryProducts({
      key,
      subcatid,
      jwttoken,
    });
  ...
  const closeSubPackModal = () => {
    setSubpacks(null);
    setSubPackModal(false);
  };
